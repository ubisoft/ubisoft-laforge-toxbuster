...
class BenchmarkerConfig(NamedTuple):
    '''
    All configurations related to the Benchmarker.
    ...
    # Save Folder Path
    save_folder_path: str = ""
    metric_result_save_file_name: str = ""
    predicted_labels_save_file_name : str = ""

    # Output info
    confidence_levels = False
    per_class_confidence_levels = False

    @staticmethod
    def from_config_file(config_file: str):
        '''
        Initializes the BenchmarkerConfig from a json config file.
        ...
        with open(config_file) as f:
            config = json.load(f)
        return BenchmarkerConfig.from_config(config)

    @staticmethod
    def from_config_files(config_files: List[str]):
        '''
        Initializes the BenchmarkerConfig from multiple json config files.
        ...
        return BenchmarkerConfig.from_configs(configs)


    @staticmethod
    def from_config(config: dict):
        '''
        Initializes the Benchmarker Config from a dictionary that possibly contains more info than necessary.
        ...
        '''
        config_fields = BenchmarkerConfig._fields
        ...
        return BenchmarkerConfig(**filtered_config)
    ...
    def from_configs(configs: List[dict]):
        ...
        return BenchmarkerConfig.from_config(final_config)
    ...
    def save(self, output_file: str):
        '''
        Given the BenchmarkerConfig, saves appropriate information into a JSON file.
